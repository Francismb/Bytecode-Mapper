 ...
 */
public class FloatFeatureProducer extends FeatureProducer<ClassNode>{
    ...
    public void visitClassNode(final ClassNode classNode) {
        final Feature<ClassNode> feature = new Feature<>(classNode, "FLOATS", 0.0);
        for (final FieldNode fieldNode : classNode.fields) {
            if (Type.isNot(fieldNode.access, Opcodes.ACC_STATIC) && fieldNode.desc.equals("F")) {
                feature.value++;
            }
        }
