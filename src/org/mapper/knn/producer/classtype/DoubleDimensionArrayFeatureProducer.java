 ...
 */
public class DoubleDimensionArrayFeatureProducer extends FeatureProducer<ClassNode> {
    ...
    public void visitClassNode(final ClassNode classNode) {
        final Feature<ClassNode> feature = new Feature<>(classNode, "ARRAY2", 0.0);
        for (final FieldNode fieldNode : classNode.fields) {
            if (Type.isNot(fieldNode.access, Opcodes.ACC_STATIC) && (fieldNode.desc.length() - fieldNode.desc.replace("[", "").length()) == 2) {
                feature.value++;
            }
        }
