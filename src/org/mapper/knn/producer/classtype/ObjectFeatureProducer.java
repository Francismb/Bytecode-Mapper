 ...
 */
public class ObjectFeatureProducer extends FeatureProducer<ClassNode>{
    ...
    public void visitClassNode(final ClassNode classNode) {
        final Feature<ClassNode> feature = new Feature<>(classNode, "OBJECTS", 0.0);
        for (final FieldNode fieldNode : classNode.fields) {
            if (Type.isNot(fieldNode.access, Opcodes.ACC_STATIC) && fieldNode.desc.startsWith("L") && fieldNode.desc.endsWith(";")) {
                feature.value++;
            }
        }
