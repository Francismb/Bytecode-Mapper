...
public class FeatureSpace<T> {
     ...
     */
    public void compute(final Instance<T> instance) {
        /**
         * Reset then Calculate the distances between the instance and data
         * Calculate the distance between instance attributes and the features attributes
         */
        for (int x = 0; x < features.length; x++) {
            for (int y = 0; y < features[x].length; y++) {
                features[x][y].distance = 0;
                if (instance.attributes.containsKey(features[x][y].attribute.key)) {
                    features[x][y].distance += Math.abs(
                            instance.attributes.get(features[x][y].attribute.key) - features[x][y].attribute.value
                    );
                } else {
    ...
    /**
     * Created to get the most common <T> neighbor
     *
